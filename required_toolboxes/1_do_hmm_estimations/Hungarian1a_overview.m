...
% load valid_inferences.mat
% iters=valid_inferences;
valid_inferences = 1:15;
iters=valid_inferences;

% iters = iters(iters<60);
...
for i=1:numel(iters)
    ...
    hmmvars_i=load(sprintf('movie2/HMMrun_K12_rep_%d.mat', iters(i)));
    allw_i=[];  
    for ii=1:NSTATES
    ...
    for j=setxor(1:numel(iters), i)
        ...
        % load the states of i
        hmmvars_j=load(sprintf('movie2/HMMrun_K12_rep_%d.mat', iters(j)));
        ...
        reshuffled_vpath_j = [];
        tmp3=sortrows([1:NSTATES; reassignments]',2);
        alternative = tmp3(:,1);
