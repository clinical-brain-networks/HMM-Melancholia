...
function post_hmm_2_make_state_figures_shirer(which_file, varargin)
...
output_filename = [my_p filesep 'figures' filesep 'Fig1_v2_shirer_states_' my_f '.jpg'];
...
roi_base = '4D_shirer_atlas_resampled.nii';
...
B2POS = [0 0.1 0.9*0.2 0.9] + [0.15 0 0 0];
B1POS = [0.2 0.1 0.9*0.23 0.9] + [0.15 0 0 0];
BARPOS = [0 0 0.53 0.095]  + [0.0375 0 0 0];
% TEXTCOLOR = 'w';
% TEXTPOS =
...
for state=1:NSTATES
    ...
    if BOUTLINE
        ah0=axes('parent',fh(state),'position',B1POS);
        t1_outline=0.5 * squeeze(t1_outline_data(sl,25:215,02:173))';
    ...
    ah1=axes('parent',fh(state),'position',B1POS);
    t1data=squeeze(t1_data(sl,25:215,02:173))';
    ...
    for i=1:14
        ...
        this_w = allw(i, state);
        ah2=axes('parent',fh(state),'position',B1POS);
        % keyboard;
        roidata=squeeze(roi_data{i}(sl,25:215,02:173))';
    ...
    if BOUTLINE
        ...
        ah1axoutl=axes('parent',fh(state),'position',B2POS);
    ...
    ah1ax=axes('parent',fh(state),'position',B2POS);
    ...
    for i=1:14
        ...
        this_w = allw(i, state);
        ah2ax=axes('parent',fh(state),'position',B2POS);
...
%             roidata=squeeze(roi_data{i}(:,:,this_sl))';
%             im2ax=imagesc(ah2ax,(roidata>thr_fig)*this_w);
%             axis xy
%             colormap(ah2ax,cmap_activities);
%             set(ah2ax,'clim',clims);
