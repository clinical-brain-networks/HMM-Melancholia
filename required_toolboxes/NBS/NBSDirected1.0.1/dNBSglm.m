function test_stat=dNBSglm(varargin)
%dNBSglm  Permutation-based non-parametric inference using the general
%linear model (GLM). Vectorial implementation, in the sense that a for loop 
%is not used for each seperate GLM. 
%
%   Test_Stat=dNBSglm(GLM) operates on each GLM defined by the structure
%   GLM. 
%
%   Test_Stat=dNBSglm(GLM,H) attempts to write out progress to uiwaitbar 
%   with handle H.
%
