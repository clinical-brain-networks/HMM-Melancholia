function dNBSrun(varargin)
%dNBSrun Reads user inputs, performs basic error checking and then performs 
%operations specified by user inputs.
%
%   dNBSrun(UI) operates on the user inputs in the structure UI.
%
%   dNBSrun(UI,S) attempts to write progress to the graphical user interface
...
if exist(fileparts(UI.matrices.ui),'dir')
    nbs.NBS.FolderConnMat = fileparts(UI.matrices.ui);
    nbs.NBS.FolderConnMatFile = UI.matrices.ui;
...
[nbs.STATS.thresh,UI.thresh.ok]=read_threshold(UI.thresh.ui,DIMS);
if  UI.thresh.ok == 0
     UI.thresh.ok=1;
    ...
    %If singel Threshold construct threshold for as it would be an range of thresholds
    nbs.STATS.thresh = [nbs.STATS.thresh nbs.STATS.thresh nbs.STATS.thresh];
end
...
if repeat
    ...
    if (DIMS.nodes*(DIMS.nodes-1)/2)*(nbs.GLM.perms)<Limit
        ...
        nbs.STATS.test_stat=dNBSglm(nbs.GLM,S.OUT.waitbar);  
...
if strcmp(UI.method.ui,'Run NBS')
   ...
   %  keyboard;
    try [nbs.NBS.n,nbs.NBS.con_mat,nbs.NBS.pval]=NBSdirected(nbs.STATS,S.OUT.ls,nbs.GLM,nbs.NBS);
...
function [y,ok,DIMS]=read_matrices(Name)
    ...
    if ok==1
        ...
        %DIMS.FolderConnectivityMatrices = Name;
    else
        %Number of nodes
        DIMS.nodes=0;
        %Number of matrices
...
%Read threshold
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [contrast,ok]=read_threshold(Name,DIMS)
ok=1; 
data=readUI(Name);
if ~isempty(data)
    [nr,nc,ns]=size(data);
    %Check if it's a running trheshold if not set ok to 0 and check if ist
    %a singel threshold
    if nr==1 && nc== 3 &&  ns==1 && isnumeric(data) 
        contrast=data; 
    else
        ok=0; contrast=[];
    end
